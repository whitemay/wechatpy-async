...
class WeChatGroup(BaseWeChatAPI):
    async def create(self, name):
        ...
        使用示例::
            ...
            res = await client.group.create('New Group')

        """
        name = to_text(name)
        return await self._post("groups/create", data={"group": {"name": name}})
    ...
    async def get(self, user_id=None):
        ...
        使用示例::
            ...
            client = WeChatClient('appid', 'secret')
            group = await client.group.get('openid')

        """
        if user_id is None:
            res = await self._get("groups/get", result_processor=lambda x: x["groups"])
        else:
    ...
    async def update(self, group_id, name):
        ...
        使用示例::
            ...
            client = WeChatClient('appid', 'secret')
            res = await client.group.update(1234, 'New Name')

        """
        name = to_text(name)
        return await self._post("groups/update", data={"group": {"id": int(group_id), "name": name}})

    async def move_user(self, user_id, group_id):
        """
        移动用户分组
        ...
        使用示例::
            ...
            client = WeChatClient('appid', 'secret')
            res = await client.group.move_user('openid', 1234)
    ...
    async def delete(self, group_id):
        ...
        使用示例::
            ...
            client = WeChatClient('appid', 'secret')
            res = await client.group.delete(1234)

        """
        return await self._post("groups/delete", data={"group": {"id": group_id}})
