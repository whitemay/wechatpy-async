...
class WeChatQRCode(BaseWeChatAPI):
    async def create(self, qrcode_data):
        """
        创建二维码
        ...
        """
        return await self._post("qrcode/create", data=qrcode_data)

    async def show(self, ticket):
        """
        通过ticket换取二维码
        ...
        async with self._client.get(url="https://mp.weixin.qq.com/cgi-bin/showqrcode", params={"ticket": ticket}) as response:
