...
class WeChatDraft(BaseWeChatAPI):
    async def add(self, articles):
        ...
        return await self._post("draft/add", data={"articles": articles_data})

    async def get(self, media_id: str) -> dict:
        """
        获取草稿
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("draft/get", data={"media_id": media_id})

    async def delete(self, media_id: str) -> dict:
        """
        删除草稿
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("draft/delete", data={"media_id": media_id})

    async def update(self, media_id, index, articles) -> dict:
        """
        修改草稿
        开发者可通过本接口对草稿进行修改。
        详情请参考
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("draft/update", data={"media_id": media_id, "index": index, "articles": articles})
    ...
    async def count(self) -> dict:
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("draft/count")

    async def batchget(self, offset=0, count=20, no_content=0) -> dict:
        """
        获取草稿列表
        新增草稿之后，开发者可以获取草稿的列表。
        详情请参考
