...
class WeChatUser(BaseWeChatAPI):
    async def get(self, user_id, lang="zh_CN"):
        """
        ...
        return await self._get("user/info", params={"openid": user_id, "lang": lang})

    async def get_followers(self, first_user_id=None):
        """
        ...
        return await self._get("user/get", params=params)

    async def iter_followers(self, first_user_id=None):
        """
        获取所有的用户openid列表
        ...
        """
        while True:
            follower_data = await self.get_followers(first_user_id)
    ...
    async def update_remark(self, user_id, remark):
        """
        设置用户备注名
        ...
        """
        return await self._post("user/info/updateremark", data={"openid": user_id, "remark": remark})

    async def get_group_id(self, user_id):
        """
        ⚠️已废弃
        获取用户所在分组 ID
        ...
        使用示例::
            ...
            client = WeChatClient('appid', 'secret')
            group_id = await client.user.get_group_id('openid')
    ...
    async def get_batch(self, user_list):
        """
        批量获取用户基本信息
    ...
    async def change_openid(self, from_appid, openid_list):
