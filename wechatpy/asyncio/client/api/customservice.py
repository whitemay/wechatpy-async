...
class WeChatCustomService(BaseWeChatAPI):
    ...
    async def add_account(self, account, nickname, password):
        """
        添加客服账号
        详情请参考
    ...
    async def delete_account(self, account):
        ...
        """
        params_data = [
            f"access_token={quote(await self.access_token())}",
            f"kf_account={quote(to_binary(account), safe=b'/@')}",
        ]
        params = "&".join(params_data)
        return await self._get("kfaccount/del", params=params)
    ...
    async def get_accounts(self):
        ...
        :return: 客服账号列表
        """
        res = await self._get("getkflist", result_processor=itemgetter("kf_list"))
        return res

    async def upload_headimg(self, account, media_file):
        """
        设置客服帐号的头像
        详情请参考
    ...
    async def get_online_accounts(self):
        """
        获取在线客服接待信息
        详情请参考
    ...
    async def create_session(self, openid, account, text=None):
        """
        多客服创建会话
        详情请参考
        ...
        return await self._post("kfsession/create", data=data)

    async def close_session(self, openid, account, text=None):
        """
        多客服关闭会话
        详情请参考
        https://developers.weixin.qq.com/doc/offiaccount/Customer_Service/Session_control.html
        ...
        """
        data = optionaldict(openid=openid, kf_account=account, text=text)
        return await self._post("kfsession/close", data=data)

    async def get_session(self, openid):
        """
    ...
    async def get_session_list(self, account):
        """
        获取客服的会话列表
        详情请参考
        https://developers.weixin.qq.com/doc/offiaccount/Customer_Service/Session_control.html
    ...
    async def get_wait_case(self):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("kfsession/getwaitcase")

    async def get_records(self, start_time, end_time, msgid=1, number=10000):
        """
        获取客服聊天记录
        详情请参考
