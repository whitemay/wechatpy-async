...
class WeChatMisc(BaseWeChatAPI):
    async def short_url(self, long_url):
        ...
        """
        return await self._post("shorturl", data={"action": "long2short", "long_url": long_url})
    ...
    async def get_wechat_ips(self):
        ...
        """
        res = await self._get("getcallbackip", result_processor=itemgetter("ip_list"))
        return res

    async def check_network(self, action="all", operator="DEFAULT"):
        """
        网络检测
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("callback/check", data={"action": action, "check_operator": operator})
