...
class WeChatMarketing(BaseWeChatAPI):
    ...
    async def add_user_action_sets(self, _type, name, description, version="v1.0"):
        """
        创建数据源
    ...
    async def get_user_action_sets(self, user_action_set_id, version="v1.0"):
        """
        获取数据源信息
    ...
    async def add_user_actions(self, actions=(), version="v1.0"):
        """
        回传数据
        ...
        :param version: 版本号 v1.0
        """
        return await self._post("user_actions/add", params={"version": version}, json={"actions": actions})

    async def get_user_action_set_reports(
        self,
        user_action_set_id,
        start_date=None,
        end_date=None,
