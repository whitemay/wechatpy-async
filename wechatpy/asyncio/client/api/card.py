...
class WeChatCard(BaseWeChatAPI):
    ...
    async def create(self, card_data):
        """
        创建卡券
        ...
        :return: 创建的卡券 ID
        """
        result = await self._post("card/create", data=card_data, result_processor=itemgetter("card_id"))
        return result

    async def batch_add_locations(self, location_data):
        """
        ⚠️已废弃
        批量导入门店信息
    ...
    async def batch_get_locations(self, offset=0, count=0):
        """
        ⚠️已废弃
        批量获取门店信息
        """
        return await self._post("card/location/batchget", data={"offset": offset, "count": count})
    ...
    async def create_qrcode(self, qrcode_data):
        """
        创建卡券二维码
    ...
    async def create_landingpage(self, buffer_data):
        """
        创建货架
        ...
        """
        result = await self._post("card/landingpage/create", data=buffer_data)
        return result

    async def get_html(self, card_id):
        """
        图文消息群发卡券
    ...
    async def consume_code(self, code, card_id=None):
        """
        消耗 code
        ...
        return await self._post("card/code/consume", data=card_data)

    async def decrypt_code(self, encrypt_code):
        """
        解码加密的 code
    ...
    async def delete(self, card_id):
        ...
        """
        return await self._post("card/delete", data={"card_id": card_id})

    async def get_code(self, code, card_id=None, check_consume=True):
        """
        查询 code 信息
        ...
        return await self._post("card/code/get", data=card_data)

    async def get_card_list(self, openid, card_id=None):
        """
        ...
        if card_id:
            card_data["card_id"] = card_id
        return await self._post("card/user/getcardlist", data=card_data)

    async def batch_get(self, offset=0, count=50, status_list=None):
        """
        批量查询卡券信息
        ...
        return await self._post("card/batchget", data=card_data)
    ...
    async def get(self, card_id):
        ...
        """
        result = await self._post("card/get", data={"card_id": card_id}, result_processor=itemgetter("card"))
        return result

    async def update_code(self, card_id, old_code, new_code):
        """
        更新卡券 code
    ...
    async def invalid_code(self, code, card_id=None):
        """
        设置卡券失效
        ...
        if card_id:
            card_data["card_id"] = card_id
        return await self._post("card/code/unavailable", data=card_data)

    async def update(self, card_data):
        """
        更新卡券信息
        ...
        """
        return await self._post("card/update", data=card_data)

    async def set_paycell(self, card_id, is_open):
        """
        设置买单接口
        ...
        """
        return await self._post("card/paycell/set", data={"card_id": card_id, "is_open": is_open})

    async def set_test_whitelist(self, openids=None, usernames=None):
        """
        设置卡券测试用户白名单
        ...
        return await self._post("card/testwhitelist/set", data={"openid": openids, "username": usernames})

    async def activate_membercard(self, membership_number, code, **kwargs):
        """
        激活会员卡 - 接口激活方式
        ...
        kwargs["code"] = code
        return await self._post("card/membercard/activate", data=kwargs)

    async def update_membercard(self, code, card_id, **kwargs):
        """
        更新会员信息
        ...
        return await self._post("card/membercard/updateuser", data=kwargs)

    async def get_membercard_user_info(self, card_id, code):
        """
        查询会员卡的会员信息
    ...
    async def add_pay_giftcard(self, base_info, extra_info, is_membercard):
        """
    ...
    async def del_pay_giftcard(self, rule_id):
        """
        删除支付后投放卡券的规则
    ...
    async def get_pay_giftcard(self, rule_id):
        """
        查询支付后投放卡券的规则
    ...
    async def batch_get_pay_giftcard(self, effective=True, offset=0, count=10):
        """
        批量查询支付后投放卡券的规则
    ...
    async def update_movie_ticket(
        ...
        return await self._post("card/movieticket/updateuser", data=ticket)

    async def checkin_boardingpass(
        self,
        code,
        passenger_name,
        seat_class,
        ...
        if card_id:
            data["card_id"] = card_id
        return await self._post("card/boardingpass/checkin", data=data)

    async def update_luckymoney_balance(self, code, balance, card_id=None):
        """
        ⚠️已废弃
        更新红包余额
        """
        ...
        if card_id:
            card_data["card_id"] = card_id
        return await self._post("card/luckymoney/updateuserbalance", data=card_data)

    async def get_redirect_url(self, url, encrypt_code, card_id):
        """
        获取卡券跳转外链
        """
    ...
    async def deposit_code(self, card_id, codes):
        """
        导入code
        """
        card_data = {"card_id": card_id, "code": codes}
        return await self._post("card/code/deposit", data=card_data)

    async def get_deposit_count(self, card_id):
        """
        查询导入code数目
        """
        card_data = {
        ...
        return await self._post("card/code/getdepositcount", data=card_data)

    async def check_code(self, card_id, codes):
        """
        核查code
        """
        card_data = {"card_id": card_id, "code": codes}
        return await self._post("card/code/checkcode", data=card_data)

    async def modify_stock(self, card_id, n):
        """
        修改库存
        """
        if n == 0:
        ...
        return await self._post("card/modifystock", data=card_data)

    async def get_activate_url(self, card_id, outer_str=None):
        """
    ...
    async def get_activate_info(self, activate_ticket):
        """
        获取用户开卡时提交的信息
    ...
    async def set_activate_user_form(self, card_id, **kwargs):
        """
        设置开卡字段接口
        ...
        return await self._post("card/membercard/activateuserform/set", data=kwargs)
