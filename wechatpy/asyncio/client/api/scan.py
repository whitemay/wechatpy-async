...
class WeChatScan(BaseWeChatAPI):
    ...
    async def get_merchant_info(self):
        ...
        使用示例::
            ...
            info = await client.scan.get_merchant_info()
        """
        return await self._get("merchantinfo/get")
    ...
    async def create_product(self, product_data):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("product/create", data=product_data)

    async def modify_product_status(self, standard, key, status):
        """
        提交审核/取消发布商品
        ...
        return await self._post("product/modstatus", data=data)

    async def publish_product(self, standard, key):
        """
        提交审核商品 shortcut 接口

        等同于调用 ``modify_product_status(standard, key, 'on')``
        """
        return await self.modify_product_status(standard, key, "on")

    async def unpublish_product(self, standard, key):
        """
        取消发布商品 shortcut 接口

        等同于调用 ``modify_product_status(standard, key, 'off')``
        """
        return await self.modify_product_status(standard, key, "off")

    async def set_test_whitelist(self, userids=None, usernames=None):
        """
        设置测试人员白名单
        ...
        return await self._post("testwhitelist/set", data=data)

    async def get_product(self, standard, key):
        """
        查询商品信息
        ...
        return await self._post("product/get", data=data)

    async def list_product(self, offset=0, limit=10, status=None, key=None):
        """
        批量查询商品信息
        ...
        return await self._post("product/getlist", data=data)
    ...
    async def update_product(self, product_data):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("product/update", data=product_data)

    async def clear_product(self, standard, key):
        """
        清除商品信息
        ...
        return await self._post("product/clear", data=data)

    async def check_ticket(self, ticket):
        """
        检查 wxticket 参数有效性
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("scanticket/check", data={"ticket": ticket})
