...
class WeChatMaterial(BaseWeChatAPI):
    async def add(self, media_type, media_file, title=None, introduction=None):
        ...
        :return: 返回的 JSON 数据包
        """
        params = {"access_token": await self.access_token(), "type": media_type}
        if media_type == "video":
        ...
        return await self._post("material/add_material", params=params, files={"media": media_file})
    ...
    async def get(self, media_id):
        ...
        return await self._post("material/get_material", data={"media_id": media_id}, result_processor=_processor)
    ...
    async def delete(self, media_id):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("material/del_material", data={"media_id": media_id})

    async def batchget(self, media_type, offset=0, count=20):
        """
        获取素材列表
        详情请参考
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("material/batchget_material", data={"type": media_type, "offset": offset, "count": count})
    ...
    async def get_count(self):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("material/get_materialcount")

    async def open_comment(self, msg_data_id, index=1):
        """
        打开已群发文章评论
        https://mp.weixin.qq.com/wiki?id=mp1494572718_WzHIY
        """
        return await self._post("comment/open", data={"msg_data_id": msg_data_id, "index": index})

    async def close_comment(self, msg_data_id, index=1):
        """
        关闭已群发文章评论
        """
        return await self._post("comment/close", data={"msg_data_id": msg_data_id, "index": index})

    async def list_comment(self, msg_data_id, index=1, begin=0, count=50, type=0):
        """
        查看指定文章的评论数据
        """
    ...
    async def markelect_comment(self, msg_data_id, index, user_comment_id):
        """
        将评论标记精选
        """
        return await self._post(
    ...
    async def unmarkelect_comment(self, msg_data_id, index, user_comment_id):
        """
        将评论取消精选
        """
        return await self._post(
    ...
    async def delete_comment(self, msg_data_id, index, user_comment_id):
        """
        删除评论
        """
        return await self._post(
    ...
    async def add_reply_comment(self, msg_data_id, index, user_comment_id, content):
        """
        回复评论
        """
        return await self._post(
    ...
    async def delete_reply_comment(self, msg_data_id, index, user_comment_id):
        """
        删除回复
        """
        return await self._post(
