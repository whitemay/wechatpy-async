...
class WeChatFreePublish(BaseWeChatAPI):
    async def submit(self, media_id: str) -> dict:
        """
        发布接口
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("freepublish/submit", data={"media_id": media_id})

    async def get(self, publish_id: str) -> dict:
        """
        发布状态轮询接口
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("freepublish/get", data={"publish_id": publish_id})

    async def delete(self, article_id: str, index: int = 0) -> dict:
        """
        删除发布
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("freepublish/delete", data={"article_id": article_id, "index": index})

    async def getarticle(self, article_id: str) -> dict:
        """
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("freepublish/getarticle", data={"article_id": article_id})

    async def batchget(self, offset: int, count: int, no_content: int = 0) -> dict:
        """
        获取成功发布列表
        开发者可以获取已成功发布的消息列表。
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("freepublish/batchget", data={"offset": offset, "count": count, "no_content": no_content})
