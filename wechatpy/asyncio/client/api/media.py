...
class WeChatMedia(BaseWeChatAPI):
    ...
    async def upload(self, media_type, media_file):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post(url="media/upload", params={"type": media_type}, files={"media": media_file})
    ...
    async def download(self, media_id):
        ...
        :return: httpx 的 Response 实例
        """
        return await self._get("media/get", params={"media_id": media_id})
    ...
    async def get_url(self, media_id):
        ...
        :return: 临时素材下载地址
        """
        return f"https://api.weixin.qq.com/cgi-bin/media/get?access_token={await self.access_token()}&media_id={media_id}"

    async def upload_video(self, media_id, title, description):
        """
    ...
    async def upload_articles(self, articles):
        """
        上传图文消息素材
        详情请参考
        http://mp.weixin.qq.com/wiki/15/5380a4e6f02f2ffdc7981a8ed7a40753.html
        ...
        return await self._post("media/uploadnews", data={"articles": articles_data})
