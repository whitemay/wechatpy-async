...
class WeChatInvoice(BaseWeChatAPI):
    ...
    async def create_card(self, base_info, payee, invoice_type, detail=None):
        """
        创建发票卡券模板
    ...
    async def set_auth_field(self, user_field, biz_field):
        """
        设置授权页字段信息
    ...
    async def get_auth_data(self, s_pappid, order_id):
        """
        查询授权数据
    ...
    async def reject_insert(self, s_pappid, order_id, reason, redirect_url=None):
        """
        拒绝用户的开发票请求
    ...
    async def insert(self, order_id, card_id, appid, card_ext):
        """
        将电子发票卡券插入用户卡包
    ...
    async def get_pdf(self, s_media_id):
        """
        查询已上传的 PDF
    ...
    async def update_status(self, card_id, code, reimburse_status):
        """
        更新发票卡券的状态
    ...
    async def set_contact(self, phone, time_out):
        """
    ...
    async def set_pay_mch(self, mchid, s_pappid):
        """
        关联商户号与开票平台，设置支付后开票
    ...
    async def get_reimburse(self, card_id, encrypt_code):
        """
        报销方查询发票信息
    ...
    async def update_reimburse(self, card_id, encrypt_code, reimburse_status):
        """
        报销方更新发票信息
    ...
    async def batch_update_reimburse(self, openid, reimburse_status, invoice_list):
        """
        报销方批量更新发票信息
    ...
    async def get_select_title_url(self, attach=None):
        """
        获取商户专属开票链接
    ...
    async def scan_title(self, scan_text):
        """
        根据扫描码，获取用户发票抬头
