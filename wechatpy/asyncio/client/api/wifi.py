...
class WeChatWiFi(BaseWeChatAPI):
    ...
    async def list_shops(self, page_index=1, page_size=20):
        """
        获取门店列表
    ...
    async def get_shop(self, shop_id=0):
        """
        查询门店的WiFi信息
        http://mp.weixin.qq.com/wiki/15/bcfb5d4578ea818b89913472cf2bbf8f.html
    ...
    async def add_device(self, shop_id, ssid, password, bssid):
        """
        添加设备
    ...
    async def list_devices(self, shop_id=None, page_index=1, page_size=20):
        """
        查询设备
        ...
        res = await self._post("device/list", data=data, result_processor=lambda x: x["data"])
        return res

    async def delete_device(self, bssid):
        """
        删除设备
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("device/delete", data={"bssid": bssid})

    async def get_qrcode_url(self, shop_id, img_id):
        """
        获取物料二维码图片网址
    ...
    async def set_homepage(self, shop_id, template_id, url=None):
        """
        设置商家主页
        ...
        if url:
            data["struct"] = {"url": url}
        return await self._post("homepage/set", data=data)

    async def get_homepage(self, shop_id):
        """
        查询商家主页
    ...
    async def list_statistics(self, begin_date, end_date, shop_id=-1):
        """
        Wi-Fi数据统计
