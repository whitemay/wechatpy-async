...
class MerchantExpress(BaseWeChatAPI):
    ...
    async def add(self, delivery_template):
        return await self._post("merchant/express/add", data={"delivery_template": delivery_template})
    ...
    async def get_all(self):
        res = await self._get("merchant/express/getall", result_processor=lambda x: x["template_info"])
        return res
