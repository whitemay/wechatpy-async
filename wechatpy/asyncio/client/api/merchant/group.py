...
class MerchantGroup(BaseWeChatAPI):
    ...
    async def update_product(self, group_id, product):
        return await self._post("merchant/group/productmod", data={"group_id": group_id, "product": product})

    async def get_all(self):
        res = await self._get("merchant/group/getall", result_processor=lambda x: x["groups_detail"])
        return res
