...
class WeChatDevice(BaseWeChatAPI):
    ...
    async def send_message(self, device_type, device_id, user_id, content):
        """
        主动发送消息给设备
        详情请参考
    ...
    async def send_status_message(self, device_type, device_id, user_id, msg_type, device_status):
        """
        第三方主动发送设备状态消息给微信终端
        详情请参考
    ...
    async def create_qrcode(self, device_ids):
        """
        获取设备二维码
        详情请参考
    ...
    async def bind(self, ticket, device_id, user_id):
        """
        绑定设备
        详情请参考
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("bind", data={"ticket": ticket, "device_id": device_id, "openid": user_id})

    async def unbind(self, ticket, device_id, user_id):
        """
        解绑设备
        详情请参考
        https://iot.weixin.qq.com/wiki/new/index.html?page=3-4-7
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("unbind", data={"ticket": ticket, "device_id": device_id, "openid": user_id})

    async def compel_bind(self, device_id, user_id):
        """
        强制绑定用户和设备
        详情请参考
        https://iot.weixin.qq.com/wiki/new/index.html?page=3-4-7
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("compel_bind", data={"device_id": device_id, "openid": user_id})

    force_bind = compel_bind

    async def compel_unbind(self, device_id, user_id):
        """
        强制解绑用户和设备
        详情请参考
        https://iot.weixin.qq.com/wiki/new/index.html?page=3-4-7
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("compel_unbind", data={"device_id": device_id, "openid": user_id})

    force_unbind = compel_unbind

    async def get_stat(self, device_id):
        """
        设备状态查询
        详情请参考
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("get_stat", params={"device_id": device_id})

    async def verify_qrcode(self, ticket):
        """
        验证二维码
        详情请参考
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("verify_qrcode", data={"ticket": ticket})

    async def get_user_id(self, device_type, device_id):
        """
        获取设备绑定openID
        详情请参考
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("get_openid", params={"device_type": device_type, "device_id": device_id})

    get_open_id = get_user_id

    async def get_binded_devices(self, user_id):
        """
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("get_bind_device", params={"openid": user_id})

    get_bind_device = get_binded_devices

    async def get_qrcode(self, product_id=1):
        """
        获取deviceid和二维码
        详情请参考
        https://iot.weixin.qq.com/wiki/new/index.html?page=3-4-4
        ...
        return await self._get("getqrcode", params=params)

    async def authorize(self, devices, op_type=1):
        """
        设备授权
        详情请参考
