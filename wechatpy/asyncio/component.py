...
class BaseWeChatComponent:
    ...
    async def _request(self, method, url_or_endpoint, **kwargs):
        ...
        if isinstance(kwargs["params"], dict) and "component_access_token" not in kwargs["params"]:
            kwargs["params"]["component_access_token"] = await self.access_token()
    ...
    async def _fetch_access_token(self, url, data):
        ...
        expires_in = result.get("expires_in", 7200)  # Default to 7200 if not present
        self.session.set(
...
class WeChatComponent(BaseWeChatComponent):
    ...
    async def get_pre_auth_url(self, redirect_uri):
        redirect_uri = quote(redirect_uri, safe=b"")
        res = await self.create_preauthcode()
        return f"{self.PRE_AUTH_URL}?component_appid={self.component_appid}&pre_auth_code={res['pre_auth_code']}&redirect_uri={redirect_uri}"

    async def get_pre_auth_url_m(self, redirect_uri):
        """
        ...
        res = await self.create_preauthcode()
        return f"{url}component_appid={self.component_appid}&pre_auth_code={res['pre_auth_code']}&redirect_uri={redirect_uri}"
    ...
    async def query_auth(self, authorization_code):
        ...
        if (
            ...
            expires_in = result["authorization_info"].get("expires_in", 7200)  # Default to 7200 if not present
    ...
    async def get_client_by_appid(self, authorizer_appid):
        ...
        if not access_token:
            ...
            access_token_key = f"{authorizer_appid}_access_token"
            expires_in = ret.get("expires_in", 7200)  # Default to 7200 if not present
            self.session.set(access_token_key, access_token, expires_in)
    ...
    async def parse_message(self, msg, msg_signature, timestamp, nonce):
        ...
        :params nonce: 随机数
        """
        content = await self.crypto.decrypt_message(msg, msg_signature, timestamp, nonce)
        ...
        if msg.type == "component_verify_ticket":
            ...
            msg.query_auth_result = await self.query_auth(msg.authorization_code)
        return msg

    async def get_component_oauth(self, authorizer_appid):
        """
        代公众号 OAuth 网页授权
...
class ComponentOAuth:
    ...
    async def get_authorize_url(self, redirect_uri, scope="snsapi_base", state=""):
        """
    ...
    async def fetch_access_token(self, code):
        ...
        res = await self._get(
            ...
            params={
                ...
                "component_access_token": await self.component.access_token(),
                "code": code,
    ...
    async def _handle_result(self, res, method=None, url=None, **kwargs):
        ...
        if "errcode" in result and result["errcode"] != 0:
            ...
            if self.component.auto_retry and errcode in (
                ...
                kwargs["params"]["component_access_token"] = await self.component.access_token()
                return await self._request(method=method, url_or_endpoint=url, **kwargs)
            elif errcode == WeChatErrorCode.OUT_OF_API_FREQ_LIMIT.value:
                # api freq out of limit
                raise APILimitedException(errcode, errmsg, client=self, request=res.request, response=res)
