...
class WeChatSchedule(BaseWeChatAPI):
    async def add(
        ...
        return await self._post("oa/schedule/add", data=data, result_processor=op.itemgetter("schedule_id"))
    ...
    async def update(
        ...
        return await self._post("oa/schedule/update", data=data)

    async def get(self, schedule_ids):
        """
        获取日程
    ...
    async def delete(self, schedule_id):
        ...
        :param schedule_id: 日程ID
        """
        return await self._post("oa/schedule/del", data={"schedule_id": schedule_id})

    async def get_by_calendar(self, calendar_id, offset=0, limit=500):
        """
        获取日历下的日程列表
    ...
    async def del_users(self, schedule_id, users):
        """
        删除日程参与者
        ...
        return await self._post("oa/schedule/del_attendees", data={"schedule_id": schedule_id, "attendees": attendees})
