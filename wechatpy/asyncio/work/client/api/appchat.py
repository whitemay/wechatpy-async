...
class WeChatAppChat(BaseWeChatAPI):
    ...
    async def create(self, chat_id=None, name=None, owner=None, user_list=None):
        """
        创建群聊会话
        ...
        return await self._post("appchat/create", data=data)
    ...
    async def get(self, chat_id):
        ...
        :return: 会话信息
        """
        res = await self._get("appchat/get", params={"chatid": chat_id})
        return res["chat_info"]

    async def update(self, chat_id, name=None, owner=None, add_user_list=None, del_user_list=None):
        """
        修改群聊会话
        ...
        return await self._post("appchat/update", data=data)

    async def send(self, chat_id, msg_type, **kwargs):
        """
        应用推送消息
        ...
        return await self._post("appchat/send", data=data)

    async def send_text(self, chat_id, content, safe=0):
        """
        发送文本消息
        ...
        :return:
        """
        return await self.send(chat_id, "text", safe=safe, content=content)
