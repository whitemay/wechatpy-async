...
class WeChatDepartment(BaseWeChatAPI):
    async def create(self, name, parent_id=1, order=None, id=None, name_en=None):
        """
        创建部门
        ...
        return await self._post("department/create", data=data)

    async def update(self, id, name=None, parent_id=None, order=None, name_en=None):
        """
        更新部门
        ...
        return await self._post("department/update", data=data)
    ...
    async def delete(self, id):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("department/delete", params={"id": id})

    async def list(self, id=None):
        """
        获取指定部门列表
        ...
        """
        if id is None:
            res = await self._get("department/list")
        else:
            res = await self._get("department/list", params={"id": id})
        return res["department"]

    async def simple_list(self, id=None):
        """
        ...
        """
        if id is None:
            res = await self._get("department/simplelist")
        else:
            res = await self._get("department/simplelist", params={"id": id})
    ...
    async def get(self, id):
        ...
        :return: 部门信息
        """
        res = await self._get("department/get", params={"id": id})
        return res["department"]

    async def get_users(self, id, fetch_child=0, simple=True):
        """
    ...
    async def get_map_users(self, id=None, key="name", fetch_child=0):
        """
        映射员工某详细字段到 ``user_id``
        ...
        """
        ids = [id] if id is not None else [item["id"] for item in await self.list()]
        users_info = list(
            chain(*[await self.get_users(department, fetch_child=fetch_child, simple=False) for department in ids])
        )
