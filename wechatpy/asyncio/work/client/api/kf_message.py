...
class WeChatKFMessage(BaseWeChatAPI):
    ...
    * 地理位置
    ...
    async def send(self, user_id, open_kfid, msgid="", msg=None):
        """
        ...
        if msgid:
            data["msgid"] = msgid
        data.update(msg)
        return await self._post("kf/send_msg", data=data)

    async def send_text(self, user_id, open_kfid, content, msgid=""):
    ...
    async def send_image(self, user_id, open_kfid, media_id, msgid=""):
        return await self.send(
            user_id,
            open_kfid,
            msgid,
    ...
    async def send_voice(self, user_id, open_kfid, media_id, msgid=""):
        return await self.send(
            user_id,
            open_kfid,
            msgid,
    ...
    async def send_video(self, user_id, open_kfid, media_id, msgid=""):
    ...
    async def send_file(self, user_id, open_kfid, media_id, msgid=""):
        return await self.send(
            user_id,
            open_kfid,
            msgid,
    ...
    async def send_articles_link(self, user_id, open_kfid, article, msgid=""):
        articles_data = {
            "title": article["title"],
            "desc": article["desc"],
            "url": article["url"],
    ...
    async def send_msgmenu(self, user_id, open_kfid, head_content, menu_list, tail_content, msgid=""):
        return await self.send(
            user_id,
            open_kfid,
            msgid,
    ...
    async def send_location(self, user_id, open_kfid, name, address, latitude, longitude, msgid=""):
        return await self.send(
            user_id,
            open_kfid,
            msgid,
    ...
    async def send_miniprogram(self, user_id, open_kfid, appid, title, thumb_media_id, pagepath, msgid=""):
        return await self.send(
            user_id,
            open_kfid,
            msgid,
