...
class WeChatExternalContactGroupChat(BaseWeChatAPI):
    ...
    async def list_all(self, status_filter: int = 0, owner_userid_list: List = None, limit: int = 100) -> List:
        """
        ...
        cursor = None
        while True:
            result = await self.list(status_filter, owner_userid_list, cursor, limit)
            if result["errcode"] == 0:
    ...
    async def get(self, chat_id: str):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("externalcontact/groupchat/get", data={"chat_id": chat_id})
    ...
    async def statistic(
        ...
        if owner_userid_list:
            data["owner_filter"] = {"userid_list": owner_userid_list}
        return await self._post("externalcontact/groupchat/statistic", data=data)
