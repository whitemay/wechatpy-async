...
class WeChatKF(BaseWeChatAPI):
    ...
    async def sync_msg(self, token, cursor="", limit=1000):
        """
        ...
        return await self._post("kf/sync_msg", data=data)

    async def get_service_state(self, open_kfid, external_userid):
        """
        获取会话状态
        ...
        return await self._post("kf/service_state/get", data=data)

    async def trans_service_state(self, open_kfid, external_userid, service_state, servicer_userid=""):
        """
        变更会话状态
        ...
        return await self._post("kf/service_state/trans", data=data)

    async def get_servicer_list(self, open_kfid):
        """
        获取接待人员列表
        ...
        return await self._get("kf/servicer/list", params=data)

    async def add_servicer(self, open_kfid, userid_list):
        """
        添加接待人员
        添加指定客服帐号的接待人员。
        ...
        return await self._post("kf/servicer/add", data=data)

    async def del_servicer(self, open_kfid, userid_list):
        """
        删除接待人员
        从客服帐号删除接待人员
        ...
        return await self._post("kf/servicer/del", data=data)

    async def batchget_customer(self, external_userid_list):
        """
        客户基本信息获取
        ...
        return await self._post("kf/customer/batchget", data=data)
    ...
    async def get_account_list(self):
        ...
        :return: 接口调用结果
        """
        return await self._get("kf/account/list")

    async def add_contact_way(self, open_kfid, scene):
        """
        获取客服帐号链接
        ...
        return await self._post("kf/add_contact_way", data=data)

    async def get_upgrade_service_config(self):
        """
        获取配置的专员与客户群

        :return: 接口调用结果
        """
        return await self._get("kf/customer/get_upgrade_service_config")

    async def upgrade_service(self, open_kfid, external_userid, service_type, member=None, groupchat=None):
        """
        为客户升级为专员或客户群服务
        ...
        return await self._post("kf/customer/upgrade_service", data=data)

    async def cancel_upgrade_service(self, open_kfid, external_userid):
        """
        为客户取消推荐
        ...
        return await self._post("kf/customer/cancel_upgrade_service", data=data)

    async def send_msg_on_event(self, code, msgtype, msg_content, msgid=None):
        """
        ...
        return await self._post("kf/send_msg_on_event", data=data)
