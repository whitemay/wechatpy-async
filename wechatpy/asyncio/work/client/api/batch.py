...
class WeChatBatch(BaseWeChatAPI):
    ...
    async def sync_user(self, url, token, encoding_aes_key, media_id, to_invite=True):
        """
        增量更新成员
    ...
    async def replace_user(self, url, token, encoding_aes_key, media_id, to_invite=True):
        """
        全量覆盖成员
    ...
    async def replace_party(self, url, token, encoding_aes_key, media_id):
        """
        全量覆盖部门
    ...
    async def get_result(self, job_id):
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._get("batch/getresult", params={"jobid": job_id})

    async def invite(self, user=None, party=None, tag=None):
        """
        邀请成员
        ...
        return await self._post("batch/invite", data=data)
