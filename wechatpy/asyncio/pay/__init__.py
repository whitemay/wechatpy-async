...
class WeChatPay:
    ...
    def __init__(
        ...
        if mch_cert and mch_cert.endswith(".p12"):
            ...
            with open(mch_cert, 'rb') as f:
                pkcs12_content = f.read()
                ssl_context = create_ssl_context(pkcs12_content, bytearray(self.mch_id, 'utf-8'))
    ...
    async def _fetch_sandbox_api_key(self):
        ...
        response = await self._http.post(api_url, data=payload, headers=headers)
    ...
    async def _request(self, method, url_or_endpoint, **kwargs):
        ...
        return await self._handle_result(res)

    async def _handle_result(self, res):
    ...
    async def parse_refund_notify_result(self, xml):
        ...
        data = await refund_crypto.decrypt_message(xml, self.appid, self.mch_id)
        for key in (
            "total_fee",
            "settlement_total_fee",
            "refund_fee",
